...
function RoleBadge({ role }: { role: AdminRole }) {
  const map: Record<AdminRole, string> = {
    SUPER_ADMIN: 'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400',
    FINANCE: 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400',
    SUPPORT: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-400',
  }
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${map[role] ?? ''}`}>
      {role.replace('_', ' ')}
    </span>
  )
}
...
// Exact shape returned by GET /admin/auth/sessions
interface Session {
  id: string
  ipAddress: string
  userAgent: string
...
export default function ProfilePage() {
  ...
  const { mutateAsync: revokeSession } = useMutation({
    mutationFn: async (sessionId: string) => {
      await adminApi.post(`/admin/auth/sessions/${sessionId}/revoke`)
    },
    onSuccess: () => {
  ...
  const passwordChecks = PASSWORD_CHECKS.map((c) => ({ ...c, passed: c.test(newPassword) }))
  ...
  function getDeviceLabel(userAgent: string): string {
    if (/mobile|android|iphone|ipad/i.test(userAgent)) return userAgent.slice(0, 60) || 'Mobile Device'
    return userAgent.slice(0, 60) || 'Desktop Browser'
  }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>After changing your password, other active sessions will be logged out</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              {newPassword.length > 0 && (
                <ul className="mt-2 space-y-1">
                  {passwordChecks.map((c) => (
                    <li key={c.label} className={`flex items-center gap-1.5 text-xs ${c.passed ? 'text-green-600' : 'text-muted-foreground'}`}>
            ...
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={changingPassword || !allPasswordChecksPassed || newPassword !== confirmPassword || !currentPassword}
              >
                Update Password
              </Button>
            </div>
                ...
                <li key={s.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-muted">
                      {/mobile|android|iphone|ipad/i.test(s.userAgent) ? (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {s.ipAddress} &middot; {formatRelativeDate(s.createdAt)}
                      </p>
                    </div>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => handleRevokeSession(s.id)}>
                    Revoke
                  </Button>
                </li>
              ))}
        ...
        <CardContent>
          <div className="flex items-center justify-between rounded-lg border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-900/10 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Revoke all active sessions one by one from the list above</p>
            </div>
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will revoke all active sessions. You will be redirected to login.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
